...
flatten = Flatten()(bert_in)

out = Dense(1, name='output', activation='sigmoid')(flatten)

distilbert_base_model2 = Model(inputs=[input_ids, att_masks], outputs=[out])

distilbert_base_model2.compile(optimizer=Adam(learning_rate=.01), loss='binary_crossentropy', metrics=['accuracy'])

distilbert_base_model2.fit([np.array(train_input_ids), np.array(train_attention_masks)], np.array(train_outputs).reshape(-1, 1), epochs=10, batch_size=16, verbose=2, use_multiprocessing=True)

distilbert_base_model2.save('models/distilBERT_base')